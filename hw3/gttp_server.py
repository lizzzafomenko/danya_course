...
#  сервера и клиента: https://medium.com/@pikotutorial/udp-client-server-with-python-5a5498956dd5
# больше ниче не юзал
...
class GTTP_server:
    ...
    def interact_with_client(self):
        command, client_address = self.server.recvfrom(1024)
        command = command.decode()
        server_answer = self._get_client_command(command, client_address)
        self.server.sendto(server_answer.encode(), client_address)

    def _get_client_command(self, command, client_address):
        if not command:
            return 'Got no command from the client'
        ...
        if client_address in self.active_games:
            ...
            if command.split()[0] == 'HELO':
                if len(command.split()) > 1 and float(command.split()[1]):
                    self.server.settimeout(float(command.split()[1]))
                game = GuessingGame(client_address)
                self.active_games[client_address] = game
                return game.get_range()
            else:
                return f'You should start a new game (use HELO)'
...
gttp = GTTP_server(args.ip, args.port)
while True:
    gttp.interact_with_client()